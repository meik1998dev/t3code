...
const makeProviderService = Effect.fn("makeProviderService")(function* (
   ...
   * and thread tools are capabilities on that credential, so turning browser
   * access off withholds the preview tools without taking the server away.
  ...
  const prepareMcpSession = (threadId: ThreadId, providerInstanceId: ProviderInstanceId) =>
    Effect.gen(function* () {
      ...
      const orchestration = yield* mayStartThreads(threadId);
    ...
    function* (threadId, rawInput) {
      ...
      return yield* Effect.gen(function* () {
        ...
        if (!instanceInfo.enabled) {
          return yield* toValidationError(
            "ProviderService.startSession",
            `Provider instance '${resolvedInstanceId}' is disabled in Spindle settings.`,
          );
        }
