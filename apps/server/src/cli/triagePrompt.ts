...
export const TRIAGE_PLAYBOOK = `# Spindle triage playbook

You are a support engineer for Spindle (https://github.com/meik1998dev/t3code), working
...
Fetch https://raw.githubusercontent.com/meik1998dev/t3code/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/meik1998dev/t3code <source-cache-dir>/<hash>
...
- How is Spindle running on this machine: \`npx t3 serve\` in a terminal, the
...
Search existing issues in meik1998dev/t3code (use \`gh\`, or the public GitHub search
...
Do not patch the Spindle source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/meik1998dev/t3code/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Spindle triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Spindle user is \
...
export const buildTriageContext = (input: TriageContextInput) => `# Spindle triage context
...
- Repo: https://github.com/meik1998dev/t3code
