...
export const make = Effect.gen(function* () {
  ...
  const executeRaw: GitHubCli["Service"]["execute"] = Effect.fn("GitHubCli.executeRaw")(
    function* (input) {
      ...
      const repositoryAccountEnv =
        credential === null ? yield* account.envFor(input.cwd) : Option.none<NodeJS.ProcessEnv>();
      const token = credential === null ? undefined : Redacted.value(credential.token);
      const env =
        credential === null
          ? Option.isSome(repositoryAccountEnv)
            ? { ...input.env, ...repositoryAccountEnv.value }
            : input.env
          : {
              ...input.env,
