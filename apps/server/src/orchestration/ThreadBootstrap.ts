...
export interface ThreadBootstrapShape {
  /**
   * Runs a `thread.turn.start` that carries a `bootstrap`: create the thread,
   * prepare its worktree, launch the setup script, then start the turn. A
   * thread this call created is deleted again when a later step fails.
   *
   * Callers own admission (for example the startup command queue); `origin`
   * is stamped onto every event the bootstrap produces.
   */
    ...
    Effect.mapError(
      (cause) =>
        new OrchestrationDispatchCommandError({
          message:
            cause instanceof Error ? cause.message : "Failed to generate command identifier.",
          cause,
        }),
    ),
  );
