...
export interface UiProjectState {
  projectExpandedById: Record<string, boolean>;
  projectOrder: string[];
  // Logical project keys the sidebar list is scoped to; empty means "all
...
export function parsePersistedState(parsed: PersistedUiState): UiState {
  ...
  return {
    ...
    sidebarProjectScopeKeys: sanitizeProjectScopeKeys(
      parsed.sidebarProjectScopeKeys,
      parsed.sidebarProjectScopeKey,
    ),
  };
}
...
// Reads the current array key and the single key written by older builds, so
...
export function setSidebarProjectScopeKeys(
  ...
  const nextKeys = sanitizeProjectScopeKeys(projectKeys, null);
  const current = state.sidebarProjectScopeKeys;
  if (current.length === nextKeys.length && current.every((key, i) => key === nextKeys[i])) {
    return state;
  }
  return {
    ...state,
