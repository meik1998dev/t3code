...
export const BranchToolbarEnvironmentSelector = memo(function BranchToolbarEnvironmentSelector({
  ...
  if (envLocked || onEnvironmentChange === undefined) {
    return (
      <span
        className="inline-flex h-7 min-w-0 max-w-full items-center gap-1 border border-transparent px-1 text-sm font-normal text-muted-foreground/70 sm:h-5 sm:text-[11px]"
  ...
  return (
    ...
    >
      <SelectTrigger
        variant="ghost"
        size="xs"
        className="min-w-0 max-w-full font-normal px-1 sm:h-5 sm:text-[11px]"
        aria-label="Run on"
        data-composer-context-control
      >
        <EnvironmentMachineIcon
