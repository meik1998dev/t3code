    ...
    <TooltipPopup side="right" align="start" sideOffset={4} variant="glass">
      ...
      <div className="flex min-w-0 max-w-80 flex-col gap-2 px-1 py-2">
        ...
        <div className="grid gap-1.5 pl-0.5 text-xs text-muted-foreground">
          ...
          {thread.branch ? (
            <div className="flex min-w-0 items-center gap-2 font-mono text-foreground/75">
            ...
            <div className="mt-0.5 flex min-w-0 items-center gap-1.5 text-secondary-label text-xs">
              ...
              {thread.branch ? (
                <>
                  ...
                  <span className="flex min-w-0 flex-1 font-mono text-muted-foreground/40">
...
export default function Sidebar() {
  ...
  // Several projects can be scoped at once; an empty set means "all". The
  // selection lives in the persisted UI store next to the other sidebar
  ...
  const setProjectScopeKeys = useCallback(
    (keys: ReadonlySet<string>) => setPersistedProjectScopeKeys([...keys]),
    [setPersistedProjectScopeKeys],
  );
  ...
  const selectedProjectScopeItems = useMemo(
    () => projectScopeItems.filter((item) => projectScopeKeys.has(item.value)),
  ...
  const filteredProjectScopeItems = useMemo(
    () =>
      filterSidebarProjectScopeItems({
        ...
        activeScopeKeys: projectScopeKeys,
  ...
  // The single scoped project, when exactly one is scoped: the trigger shows
  // its favicon and the empty state names it.
  const scopedProjectGroup = scopedProjectGroups.length === 1 ? scopedProjectGroups[0]! : null;
  ...
  // Drop scope keys whose project disappeared (removed, or its environment
  // went away) so the scope never silently hides every thread. Wait until
  // every catalog environment has a live project snapshot: cached or
  // disconnected environments cannot establish that a project is gone.
  const allProjectSnapshotsReady = useAllEnvironmentProjectSnapshotsReady();
  useEffect(() => {
    if (!allProjectSnapshotsReady) return;
    if (projectScopeKeys.size !== scopedProjectGroups.length) {
      setProjectScopeKeys(new Set(scopedProjectGroups.map((group) => group.projectKey)));
    }
  ...
  const settledResetKey = [...projectScopeKeys].toSorted().join("\n") || "all";
            ...
            buildThreadActionMenuItems({
              ...
              projectFilter: threadProjectGroup
                ? {
                    ...
                    isActive: projectScopeKeys.has(threadProjectGroup.projectKey),
                  }
                : null,
              isPinned,
              isSettled,
        ...
        switch (clicked.value) {
          case "filter-by-project":
            ...
            if (threadProjectGroup) {
              ...
              setProjectScopeKeys(
                onlyThisProject ? new Set() : new Set([threadProjectGroup.projectKey]),
              );
            }
            return;
              ...
              projectScope={
                <Combobox
                  ...
                  onValueChange={(items) => {
                    ...
                    setProjectScopeKeys(
                      resolveSidebarProjectScopeKeys(items.map((item) => item.value)),
                    );
                  }}
                >
                  <ComboboxTrigger
                    render={
                      <SidebarHeaderIconButton
                        label={
                          scopedProjectGroups.length > 0
                            ? `Filter threads by project: ${formatSidebarProjectScopeLabel(
                                scopedProjectGroups.map((group) => group.displayName),
                              )}`
                  ...
                  >
                    ...
                    <ComboboxList>
                      {(item: (typeof projectScopeItems)[number]) => {
                        ...
                        const isScoped = project !== null && projectScopeKeys.has(item.value);
                        return (
                          <ComboboxItem
                            key={item.value}
                            hideIndicator
