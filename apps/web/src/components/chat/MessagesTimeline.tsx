      ...
      <div className="flex w-full max-w-[80%] items-center justify-end pe-1 text-xs tabular-nums opacity-0 transition-opacity duration-200 pointer-coarse:opacity-100 focus-within:opacity-100 group-hover:opacity-100">
        <div className="flex shrink-0 items-center gap-2">
          ...
          <div className="flex items-center gap-0.5">
            {canRevertAgentWork && (
              <RevertMessageButton turnCount={row.revertTurnCount} />
            )}
...
function RevertMessageButton({ turnCount }: { turnCount?: number | undefined }) {
  const ctx = use(TimelineRowCtx);
  const activity = use(TimelineRowActivityCtx);
  const checkpointAvailable = typeof turnCount === "number";
      ...
      <TooltipTrigger
        render={
          <Button
            ...
            onClick={() => {
              if (turnCount !== undefined) ctx.onRevertToTurnCount(turnCount);
            }}
...
function AssistantMessageMeta({
  ...
  /** When set, the meta row also offers revert and fork actions for this message. */
  messageId?: MessageId;
  ...
  return (
    ...
    >
      <div className="flex items-center gap-0.5">
        {messageId ? (
          <RevertMessageButton turnCount={revertTurnCount} />
        ) : null}
        <AssistantCopyButton
          message={message}
        ...
        />
        {messageId ? <MessageForkMenu messageId={messageId} /> : null}
      </div>
      {!message.streaming && (
        <Tooltip>
          <TooltipTrigger render={<p className="text-muted-foreground text-xs tabular-nums" />}>
...
function WorkEntryIcon({ name, className }: { name: WorkEntryIconName; className: string }) {
  switch (name) {
    ...
    case "t3-code":
      return <SpindleMark className={className} aria-hidden />;
    case "check":
