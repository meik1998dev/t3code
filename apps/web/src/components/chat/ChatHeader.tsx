...
export const ChatHeader = memo(function ChatHeader({
  ...
  // Narrowed, not a boolean: an empty script list must not render the control at all,
  // and `scripts` below needs the non-undefined type.
  const projectScripts = activeProjectScripts?.length ? activeProjectScripts : undefined;
  const headerActions = (
    <>
      {projectScripts && (
        <>
      ...
      {showOpenInPicker && (
        <>
          {actionsCollapsed && projectScripts && <MenuSeparator />}
          <OpenInPicker
            presentation={actionsCollapsed ? "menu" : "toolbar"}
            environmentId={activeThreadEnvironmentId}
            keybindings={keybindings}
      ...
      {activeProjectName && gitCwd && (
        <>
          {actionsCollapsed && (projectScripts || showOpenInPicker) && <MenuSeparator />}
          <GitActionsControl
            presentation={actionsCollapsed ? "menu" : "toolbar"}
            gitCwd={gitCwd}
      ...
      >
        <Menu open={actionsCollapsed && actionsOpen} onOpenChange={setActionsOpen}>
          <MenuTrigger
            className={
              actionsCollapsed &&
              (projectScripts || showOpenInPicker || (activeProjectName && gitCwd))
                ? undefined
                : "hidden"
            }
