...
describe("uiStateStore pure functions", () => {
  ...
  it("scopes several projects at once", () => {
    const scoped = setSidebarProjectScopeKeys(makeUiState(), ["a", "b", "a"]);

    expect(scoped.sidebarProjectScopeKeys).toEqual(["a", "b"]);
  });
});
...
describe("uiStateStore persistence", () => {
  ...
  it("restores the sidebar project scope across reloads", () => {
    persistState(makeUiState({ sidebarProjectScopeKeys: ["github.com/pingdotgg/t3code"] }));
