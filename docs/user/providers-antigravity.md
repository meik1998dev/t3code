...
Spindle runs Google's official Antigravity ACP agent on your selected environment.
...
**Sign in with Google** and complete the browser sign-in. Wait for Spindle to confirm
...
hostname. Only that Spindle sign-in session can finish the attempt. If it expires,
...
Use Antigravity's native `/plan` command for planning. Spindle's separate Plan mode
...
Spindle keeps conversation history and file diffs, but Antigravity cannot rewind
...
Put project skills in `.agents/skills`. Spindle also reads `.gemini/skills` and the
