...
Connect a phone, browser, or another desktop app to Spindle running on a different
...
**SSH**, and enter a host or SSH alias such as `user@example.com`. Spindle starts
...
the connection stops a server that Spindle launched; a server that was already
...
| Error                                                     | Recovery                                                                                                                                    |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `environment_link_limit_exceeded` or managed tunnel limit | Deregister an unused environment, then restart Spindle on the host.                                                                         |
...
| Expired or invalid link proof                             | Check the host's date and time, update Spindle, then restart it.                                                                            |
...
devices. For server version warnings, follow [Updating Spindle](./updating.md).
...
environment**. Spindle restarts without a local server: no local agents or terminals run, WSL
