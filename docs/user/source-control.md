...
Spindle integrates with GitHub, GitLab, Forgejo, Gitea, Bitbucket, and Azure DevOps to clone and publish
...
**Several GitHub accounts?** Sign in to each one with `gh auth login`, then tell each repository
...
[Gitea CLI (`tea`)](https://gitea.com/gitea/tea) 0.16 or later on your Spindle server.
Sign in with `fj --host https://your-server auth add-token` or `tea login add`.
Repeat for each server you use, including Codeberg.

Spindle prefers a matching `fj` login and falls back to `tea` when `fj` is unavailable
