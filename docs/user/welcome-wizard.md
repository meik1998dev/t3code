...
Spindle shows a setup flow when you open a new installation or connect to the
hosted app for the first time. Existing workspaces skip this flow.

## Connect your computers

Select one or more computers to set up. If you opened Spindle directly from a
...
- **T3 Connect** connects computers that are signed in to your account. Run
  `npx t3 connect` on each computer you want to add, then start Spindle or run
...
If Spindle cannot confirm the workspace during startup, the setup flow shows
**Still connecting** instead of opening the app. Select **Reload** to try again.

If Spindle cannot read your saved settings, it shows **Could not read settings**.
Select **Retry** after storage becomes available. Setup does not replace
unreadable settings with defaults.

## Check your agents

Spindle checks each selected computer for Claude Code and Codex. If an agent is
...
Spindle finds directories that Claude Code or Codex has used. Git repositories
...
A large or malformed history can reach the scan limit. Spindle keeps the
...
30 days. You can continue those conversations in Spindle.

Conversation import is best effort. Spindle keeps the first user prompt and the
...
conversation so Spindle does not remove user text. It reads one conversation at
