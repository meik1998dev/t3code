...
describe("resolveForkPointCheckpointRef", () => {
  ...
  it("does not resolve reasoning or system messages as fork points", () => {
    expect(
      resolveForkPointCheckpointRef(
        threadId,
        { id: MessageId.make("reasoning-1"), role: "reasoning", turnId },
        checkpoints,
      ),
    ).toBeNull();
    expect(
      resolveForkPointCheckpointRef(
        threadId,
        { id: MessageId.make("system-1"), role: "system", turnId },
        checkpoints,
      ),
    ).toBeNull();
  });
