...
export const DESKTOP_FILE_EXCLUSIONS = [
  // Spindle always passes the user's installed Claude executable to the SDK,
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ? "Spindle (Nightly)"
    : (desktopPackageJson.productName ?? "Spindle");
}
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  if (platform === "mac") {
    ...
    buildConfig.mac = {
      ...
      // electron-builder compiles the layered icon into Assets.car plus a legacy ICNS.
      icon: path.join(
        repoRoot,
        "assets",
        resolveDesktopUpdateChannel(version) === "nightly" ? "nightly" : "prod",
        "app-icon.icon",
      ),
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    private: true,
    packageManager: rootPackageJson.packageManager,
    description: "Spindle desktop build",
    author: "T3 Tools",
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  ...
  Command.withDescription("Build a desktop artifact for Spindle."),
